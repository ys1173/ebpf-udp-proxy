...
//! via a lightweight HTTP server. Reads stats from eBPF per-CPU maps,
//! AF_XDP atomic counters, and userspace atomic counters.
...
async fn metrics_handler(State(state): State<MetricsState>) -> impl IntoResponse {
    ...
    // --- AF_XDP-mode listener stats ---
    for (name, stats) in state.afxdp_stats.iter() {
        ...
        // AF_XDP-specific metrics
        write_metric(
            &mut output,
            "udp_fanout_afxdp_rx_ring_empty_total",
            name,
            "af_xdp",
            stats.rx_ring_empty.load(Relaxed),
        );
        write_metric(
            &mut output,
            "udp_fanout_afxdp_fill_ring_full_total",
            name,
            "af_xdp",
            stats.fill_ring_full.load(Relaxed),
        );
        write_metric(
            &mut output,
            "udp_fanout_afxdp_parse_errors_total",
            name,
            "af_xdp",
